...
export class AppController {
    ...
    @Put('/config')
    async updateConfig(@Query('uniq') uniq: string, @Body() config: ConfigurationDto) {
    ...
    async root(@Query('uniq') uniq: string) {
        ...
        let dto = new RecordDto(user_id, null, uniq, filial.name)
    ...
    async booking(@Param() params: any, @Res() res: Response) {
        ...
        const rec = new RecordServiceDto(serv['id'], serv['duration'], serv['tarif'], serv['name'], serv['intervalonlinebooking'])
        await this.recordService.update(params.user_id, rec)
        const employees = await this.appService.getEmployees(params.id, record['uniq'])
        // await this.recordService.update(params.user_id, {employee_id: employees[0]['id'], employee_fio: 'Любой сотрудник'})
        if (employees.length === 1) {
            await this.recordService.update(params.user_id, {employee_fio: employees[0]['fio'], employee_id: employees[0]['id']})
            RESPONSE['is_link_enabled'] = false
        }
        else
            RESPONSE['is_link_enabled'] = true
        if (record['date'] === "1970-01-01")
            await this.recordService.update(params.user_id, {date: new Date().toISOString().slice(0, 10)})
        ...
        if (employee !== null && employee !== undefined && employee !== "" && employee !== "any") {

            const data = await this.appService.getTimes1(employee, week, record['uniq'], serv, null)
            ...
            dates = Array.from(uniqDates).sort((a, b) => new Date(a as string).getTime() - new Date(b as string).getTime());

            RESPONSE['employee_fio'] = await this.recordService.getEmployeeFio(params.user_id)
        ...
        // Если сотрудник не выбран или выбран любой
            ...
            for (const employee of employees) {
                const data = await this.appService.getTimes1(employee['id'], week, record['uniq'], serv, null)
                if (data === null)
                    continue
        ...
        if (record['date'] !== null)
            RESPONSE['chosen_date'] = record['date']
        if (record['start_time'] !== null)
            RESPONSE['chosen_time'] = record['start_time']

        RESPONSE['chosen_services'] = JSON.stringify(await this.recordService.getService(params.user_id))
        RESPONSE['days'] = JSON.stringify(dates)
        if (RESPONSE['chosen_date'] !== null && RESPONSE['chosen_date'] !== undefined && employee !== 'any' && employee !== null) {
            const temp = await this.appService.getTimes1(employee, 0, record['uniq'], serv, record['date'])
    ...
    async booking_day(@Body() body: any) {
        ...
        await this.recordService.update(body.user_id, {date: body.date.split('T')[0], start_time: null, end_time: null})
        const record = await this.recordService.findOne(body.user_id)
        const serv = await this.recordService.getService(body.user_id)
        const employees = await this.appService.getEmployees(serv[0]['id'], record['uniq'])
        const response = {}
        const uniqTimes = new Set()
        for (const employee of employees) {
            const data = await this.appService.getTimes1(employee['id'], 0, record['uniq'], serv[0], body.date.split('T')[0])
            if (data === null)
                continue
    ...
    async booking_time(@Body() body: any) {
        ...
        const response = {}
        for (const employee of employees) {
            const data = await this.appService.getTimes1(employee['id'], 0, record['uniq'], serv[0], record['date'])
            if (data === null)
                continue
            response[employee['id']] = data[0]
        }
        if (record['employee_id'] === null) {
            ...
            await this.recordService.update(body.user_id, {start_time: body.time, end_time: time_end, employee_id: chosen['id'], employee_fio: chosen['fio']})
        }
        else{
    ...
    async employee_page(@Param() params: any) {
        ...
        if (record['date'] !== null || record['start_time'] !== null){
            ...
            const response = {}
            for (const employee of employees) {
                let data = await this.appService.getTimes1(employee['id'], 0, record['uniq'], serv[0], record['date'])
                if (data === null)
                    data = await this.appService.getFirstFreeTime(employee['id'], await this.configService.getWeeks(record['uniq']), record['uniq'], serv[0], record['date'])
                new_employees.push({id: employee['id'], fio: employee['fio'], position: employee['position'], avatar: employee['avatar']})
                response[employee['id']] = data[0]
            }
            let chosen = null
            for (const key in response) {
                for (const item of response[key]['times']) {
                    if (item === record['start_time'] && response[key]['date'] === record['date']) {
                        chosen = new_employees.find(employee => employee['id'] === key)
                        chosen['status'] = 'Доступен'
                        break
                    }
                }
                if (new_employees.find(employee => employee['id'] === key)['status'] !== 'Доступен') {
                    ...
                    new_employees.find(employee => employee['id'] === key)['status'] = `Ближайшее доступное время ${format(new Date(response[key]['date']), 'dd.MM.yyyy')} в ${response[key]['times'][0]}`
                }
            }
            return {
            ...
            const new_employees = []
            for (const employee of employees) {
                new_employees.push({id: employee['id'], fio: employee['fio'], position: employee['position'], avatar: employee['avatar'], status: 'Доступен'})
            }
            return {
                employees: JSON.stringify(new_employees),
                service_id: record['service_id'],
    ...
    async choose_employee(@Body() body: any) {
        ...
        const serv = await this.recordService.getService(body.user_id)
        const week = await this.configService.getWeeks(record['uniq'])
        await this.recordService.update(body.user_id, {employee_id: body.employee_id, employee_fio: body.employee_fio})
        let data = null
        if (body.employee_status !== "Доступен")
            data = await this.appService.getFirstFreeTime(body.employee_id, week, record['uniq'], serv[0], record['date'])
        else
            data = [{"date": record['date'], "times": [record['start_time']]}]
        return {employee_id: body.employee_id, date: data[0]['date'], time: data[0]['times'][0], duration: parseInt(serv[0]['duration'])}
    }
    ...
    async summary_confirm(@Body() body: any, @Res() res: Response) {
        ...
        if (new Date() >= new Date(`${record['date']}T${record['start_time']}`))
            return res.redirect(`/?uniq=${record.uniq}`)
        let client: any = await this.appService.findClient(body.phone, record['uniq'])
        if (client.phone === "") {
            await this.appService.addClient(body.phone, body.client_fio, record['uniq'])
            client = await this.appService.findClient(body.phone, record['uniq'])
        }
        await this.appService.add_record(record, client)
        const employee = await this.appService.getCurrentUser(record['employee_id'], record['uniq'])
        const tokens = employee[0]['pushtockens']
        if (tokens !== null && tokens !== undefined) {
            if (tokens.length > 0){
                tokens.forEach(token => {
                    this.appService.sendPush(token, client, record['employee_fio'], record['service_name'], record['date'], record['start_time'])
                })
            }
        }
    ...
    @Post('/exit')
    async exit(@Body() body: any, @Res() res: Response) {
        const record = await this.recordService.findOne(body.user_id)
    ...
    async employee_back(@Param() params: any, @Res() res: Response) {
        await this.recordService.update(params.user_id, {employee_id: null, employee_fio: null})
    ...
    async booking_back(@Param() params: any, @Res() res: Response) {
        await this.recordService.update(params.user_id, {employee_id: null, employee_fio: null, date: null, start_time: null, end_time: null})
        return res.redirect(`/booking/${params.service_id}/${params.user_id}`)
    }
